...
########### xx START HERE
jnt.lm <- function(dat1, dat2 = NULL, which.is.fact, alpha = 0.05, total.comp = 1){

nn <- names(testlm$model)
n.fact <- which(mm == which.is.fact)

if (!is.factor(dat1$model[n.fact])) stop("'which.is.fact' is not a factor")
